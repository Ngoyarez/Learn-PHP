...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($username_err) && empty($password_err)){
        ...
        $select = "SELECT id, username, password FROM users WHERE username = ?";
        if($stmt = mysqli_prepare($conn, $select)){
            ...
            //Check If username exists, if yes the verify password
            if(mysqli_stmt_num_rows($stmt) == 1){
                //Bind variables
                mysqli_stmt_bind_param($stmt, $id, $username, $hashed_password);
